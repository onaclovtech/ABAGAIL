...
#* @version 1.0
#*/
 class SymmetricEigenvalueDecomposition {
	/** 
	 ...
	 */
	 SymmetricEigenvalueDecomposition(Matrix a):
	 ...
	 */
	 decompose(RectangularMatrix d):
        ...
        // so we can swap rows
        RectangularMatrix u = (RectangularMatrix) self.u
        for (int i = 0 i < u.m() - 1 && swapped i++):
         ...
         // retranspose u
         self.u = (RectangularMatrix) u.transpose()
         // make the diagonal
         self.d = new DiagonalMatrix(d)
	}
	 ...
	 */
	 qrstep(RectangularMatrix d, int ia, int ib):
